...
IO.inspect(assigns)
    cond do
      assigns.status.status == "available" ->
        ~H"""
          <!-- The button to open modal -->
          <label for={"seat-modal-#{assigns.station.station_number}"} class="btn btn-info"><%= assigns.station.station_number %></label>

          <!-- Put this part before </body> tag -->
          <input type="checkbox" id={"seat-modal-#{assigns.station.station_number}"} class="modal-toggle" />
          <div class="modal modal-bottom sm:modal-middle">
            <div class="modal-box">

              <h3 class="font-bold text-lg">You have selected seat <%= assigns.station.station_number %></h3>
              <p class="py-4">Select the duration of the reservation</p>

              <form phx-submit="reserve_seat">
                <input type="hidden" name="seat_number" value={"#{assigns.station.station_number}"}>
                ...
                <p><%= assigns.response.message%></p>
                <div class="modal-action">
                  <label for={"seat-modal-#{assigns.station.station_number}"} class="btn">Close</label>
                  <button for={"seat-modal-#{assigns.station.station_number}"} class="btn" type="submit">Confirm reservation</button>
                </div>
              </form>
            </div>
          </div>
        """
      assigns.status.status == "occupied" ->
        ~H"""
          <!-- The button to open modal -->
          ...
          <div class="modal modal-bottom sm:modal-middle">
            <div class="modal-box">
              ...
              <h3 class="font-bold text-lg">You have selected seat <%= assigns.station.station_number %></h3>
              <p class="py-4">This Place is currently occupied by Badge #<%= assigns.status.reservation.badge %></p>
              ...
              <form phx-submit="cancel_seat">
                ...
                <div class="modal-action">
                  <label for={"seat-modal-#{assigns.station.station_number}"} class="btn">Close</label>
                  <button for={"seat-modal-#{assigns.station.station_number}"} class="btn" type="submit" onclick={"document.getElementById('seat-modal-#{assigns.station.station_number}').checked=false"}>Confirm cancelation</button>
                </div>
              </form>
            </div>
          </div>
        """
      assigns.status.status == "broken" ->
        ~H"""
          <!-- The button to open modal -->
        ...
        """
      assigns.status.status == "reserved" ->
        ~H"""
          <!-- The button to open modal -->
